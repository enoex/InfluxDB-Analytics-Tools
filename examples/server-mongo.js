    ...
    function getStatsAndSend() {
        ...
        async.parallel([
            ...
            function getBehaviorStats(callback){
                ...
                // ----------------------
                // Example Query : User online count
                // ----------------------
                ...
                // ----------------------
                // For number of docs in a specific collection
                // ----------------------
                // NOTE: Aggregation is used to ensure right counts for
                // sharded configurations
                db.collection(YOUR_COLLECTION).aggregate([
                    { $group: { _id: null, count: { $sum: 1 } } }
                ], function ( err, result ){
                    if(err || !result ){ 
                        logger.log('error:' + YOUR_COLLECTION, 'error: ' + err);
                        return; 
                    }
                    ...
                    if(result.length < 1){
                        logger.log('warn:roomCount', 'nothing returned',
                            ' | time: ' + ms + 'ms');
                        analytics.track({
                            group: 'app:' + YOUR_COLLECTION + ':count', queryTime: ms, 
                            value: 0
                        }); 
                        return false;
                    }
